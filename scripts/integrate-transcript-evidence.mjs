...
const fetchJson = async url => {
  const response = await fetch(url, { headers: { "user-agent": "Videha-Festival-transcript-integrator/1.0" }, signal: AbortSignal.timeout(30000) });
...
for (const record of records) {
  ...
  const body = `<nav class="crumbs" aria-label="Breadcrumb"><a href="${base}">Festival</a><span aria-hidden="true">›</span><a href="${base}accessibility/">Accessibility</a><span aria-hidden="true">›</span><a href="${base}accessibility/transcripts/">Transcripts</a><span aria-hidden="true">›</span><span aria-current="page">Machine draft</span></nav><p class="eyebrow">MACHINE ASR TRANSCRIPT DRAFT</p><h1>${esc(title)}</h1><div class="warning"><strong>Not an editorially verified transcript.</strong> This text was generated automatically from the preserved recording and may contain recognition, language-detection, spelling, segmentation or name errors. Consult the source recording for authoritative content.</div><dl class="meta"><dt>Collection</dt><dd>Part ${esc(record.part)} · ${esc(record.collectionTitle || record.collectionIdentifier)}</dd><dt>Status</dt><dd>${esc(record.status)}</dd><dt>Human verified</dt><dd>No</dd><dt>Editorial review</dt><dd>${esc(record.editorialReview || "not-reviewed")}</dd><dt>ASR model</dt><dd>${esc(record.asrModel || "")}</dd><dt>Detected language</dt><dd>${esc(record.detectedLanguage || "unknown")} ${record.languageProbability != null ? `(${esc(record.languageProbability)})` : ""}</dd><dt>Duration</dt><dd>${esc(fmt(record.durationSeconds))}</dd><dt>Source</dt><dd><a href="${esc(record.mediaUrl)}">Open preserved recording</a></dd></dl><h2>Timestamped machine draft</h2><ol class="segments">${segments || `<li><span>No speech text was emitted.</span></li>`}</ol><h2>Plain text</h2><p lang="mai-Deva">${esc(record.plainText || "")}</p><p><a href="${base}accessibility/transcripts/">← Transcript index</a></p>`;
  const dir = path.join(drafts, id);
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, "index.html"), shell(title, body, canonical));
  rows.push(`<tr><td>Part ${esc(record.part)}</td><td><a href="${canonical}">${esc(title)}</a></td><td>${esc(record.detectedLanguage || "unknown")}</td><td><span class="a11y-status pending">ASR draft · not reviewed</span></td><td><a href="${esc(record.mediaUrl)}">Source recording</a></td></tr>`);
}
...
const index = shell("Transcript index", `<nav class="crumbs" aria-label="Breadcrumb"><a href="${base}">Festival</a><span aria-hidden="true">›</span><a href="${base}accessibility/">Accessibility</a><span aria-hidden="true">›</span><span aria-current="page">Transcripts</span></nav><p class="eyebrow">TRANSCRIPT ACCESS</p><h1>Transcripts and machine drafts</h1><p><strong>${verified.length} editorially verified transcript${verified.length === 1 ? "" : "s"}</strong> are stored in the repository.</p><div class="warning"><strong>${records.length} machine ASR drafts are available separately.</strong> They are accessibility aids, not publisher captions and not human/editorially verified transcripts. Their status will remain “ASR draft” until an editor reviews the text against the recording.</div>${verified.length ? `<h2>Editorially verified transcripts</h2><ul>${verifiedRows}</ul>` : ""}<h2>Machine ASR draft corpus</h2><p>Coverage: ${records.length} of ${expected} canonical recordings across the twelve Videha Discussion & Criticism collections.</p><table class="a11y-table"><caption>Machine transcript draft register</caption><thead><tr><th>Collection</th><th>Recording</th><th>Detected language</th><th>Review status</th><th>Source</th></tr></thead><tbody>${rows.join("")}</tbody></table><p><a href="${esc(manifest.releaseUrl)}">Open machine-readable transcript release</a> · <a href="${base}accessibility/transcript-register.json">Transcript status register (JSON)</a></p>`, `${base}accessibility/transcripts/`);
await writeFile(path.join(transcripts, "index.html"), index);
register.asr.publishedDraftPages = rows.length;
await writeFile(path.join(accessibility, "transcript-register.json"), JSON.stringify(register, null, 2));
console.log(`Published ${rows.length} provenance-labelled ASR draft transcript pages; ${verified.length} editorially verified transcripts remain separately labelled.`);
